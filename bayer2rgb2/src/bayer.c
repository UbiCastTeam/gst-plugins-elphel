...
* Foundation, nc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA
*/
...
/*
dc1394error_t	dc1394_bayer_NearestNeighbor
(const uint8_t *bayer,
 uint8_t *rgb,
...
dc1394_bayer_Simple(const uint8_t *bayer, uint8_t *rgb, int sx, int sy, int tile)
{
  ...
  if ((tile > DC1394_COLOR_FILTER_MAX) ||
      (tile < DC1394_COLOR_FILTER_MIN))
    return DC1394_INVALID_COLOR_FILTER;
  // add black border
  imax = sx * sy * 3;
  for (i = sx * (sy - 1) * 3; i < imax; i++)
    ...
    for (row=0; row < 8; row++) { /* Precalculate for VNG */
        for (col=0; col < 2; col++) {
            ...
            for (cp=bayervng_terms, t=0; t < 64; t++) {
                ...
                if (FC(row+y2,col+x2) != (unsigned int) color) continue;
                diag = (FC(row,col+1) == (unsigned int) color && FC(row+1,col) == (unsigned int) color) ? 2:1;
            ...
            *ip++ = INT_MAX;
            for (cp=bayervng_chood, g=0; g < 8; g++) {
                ...
                *ip++ = (y*width + x) * 3; /* [FD] */
                color = FC(row,col);
                if (FC(row+y,col+x) != (unsigned int) color && FC(row+y*2,col+x*2) == (unsigned int) color)
                 ...
                 dc1394color_filter_t pattern)
{
    ...
    /* start - code from border_interpolate (int border) */
    {
	  ...
	  for (col=0; col < (unsigned int)width; col++) {
	    if (col== (unsigned int) border && row >= (unsigned int) border && row < (unsigned int)height - (unsigned int) border)
                    col = width-border;
                memset (sum, 0, sizeof sum);
                for (y=row-1; y != row+2; y++)
                    for (x=col-1; x != col+2; x++)
		      if (y < (unsigned int) height && x < (unsigned int)width) {
                            f = FC(y,x);
        ...
        for (left=0; left < width; left += TS-6) {
            ...
            for (row=top+2; row < top+TS-2 && row < height; row++) {
                ...
                for (col=left+2; col < left+TS-2 && col < width; col++) {
                    ...
                    for (d=0; d < 2; d++)
                        for (i=0; i < 4; i++)
                            ldiff[d][i] = ABSOLU(lab[d][tr][tc][0]-lab[d][tr][tc+dir[i]][0]);
...
/*
 * We choose the method defined in property execpted bilinear method.
 * We have all method in 8 bits
*/
dc1394error_t
